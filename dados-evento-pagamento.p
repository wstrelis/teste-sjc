...
/* ************************  Function Prototypes ********************** */
function ObtemTipo returns character 
...
------------------------------------------------------------------------------*/
    ...
    define variable in-id                               as   integer    no-undo.
    
    
    for each evenfatu
   use-index evenfat1
       where evenfatu.cd-evento     > 0
         and evenfatu.in-entidade   = "PP":
             
        if evenfatu.dt-atualizacao  < dt-alterados-desde
        then next.             
        ...
        assign XML_EVENTO.CHAVE_SISTEMA = RegistroPrimary (buffer evenfatu:handle)
               ...
               XML_EVENTO.CATEGORIA     = CATEGORIA_VALOR
               XML_EVENTO.OPERACAO      = if evenfatu.lg-cred-deb then "CREDITO" else "DEBITO"
               XML_EVENTO.TIPO          = ObtemTipo (evenfatu.in-classe-evento).
...
------------------------------------------------------------------------------*/    
    ...
    case ch-classe:
        when "G" then return "GLOSA".
        when "N" then return "PRODUCAO".
        when "F" 
     or when "J" 
     or when "I" 
     or when "S" 
     ...
     or when "P" 
     or when "L" 
     or when "U" then return "IMPOSTO".
        otherwise return "OUTROS".
    end.
