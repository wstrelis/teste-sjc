...
define temp-table XML_CONTRATANTE no-undo
    namespace-uri "http://www.thealth.com.br/ContractingParty.xsd" 
    field CODIGO as character 
        xml-node-type "ATTRIBUTE" 
    field CHAVE_SISTEMA as character 
    field NOME as character 
    field TIPO_PESSOA as character 
    field CPF_CNPJ_CEI as character .

define dataset XML_CONTRATANTES namespace-uri "http://www.thealth.com.br/ContractingParty.xsd" 
    for XML_CONTRATANTE.
...
procedure Executa:
    ...
    define variable in-qt-dias                  as   integer        no-undo.
    ...
    then do:
        assign dt-alterado-desde    = 01/01/0001.
    end.
    else do:
        assign dt-alterado-desde    = temp-parametro-entrada.dt-qt-dias.
    end.

    run Processa (input  dt-alterado-desde).

    EnviarDados (dataset XML_CONTRATANTES:handle). 
...
------------------------------------------------------------------------------*/
    ...
    for each contrat no-lock:
        
        if contrat.dt-atualizacao   < dt-alterados-desde
        then next.
        
        find first propost no-lock        
         use-index propo26
             where propost.cd-contratante       = contrat.cd-contratante
                   no-error. 
        ...
        then do:
            ...
            find first propost no-lock
             use-index propo23
                 where propost.cd-contrat-origem    = contrat.cd-contratante
                       no-error.
        end.
        ...
        create XML_CONTRATANTE.
        assign XML_CONTRATANTE.CHAVE_SISTEMA    = RegistroPrimary (buffer contrat:handle)
               XML_CONTRATANTE.CODIGO           = string (contrat.cd-contratante, '999999999')
               XML_CONTRATANTE.NOME             = contrat.nm-contratante
               XML_CONTRATANTE.TIPO_PESSOA      = if contrat.in-tipo-pessoa = 'F' then "FISICA" else "JURIDICA"
               XML_CONTRATANTE.CPF_CNPJ_CEI     = if contrat.nr-cgc-cpf <> "" then contrat.nr-cgc-cpf else contrat.nr-cei
               in-conta                         = in-conta + 1.

        if in-conta = 5000
        then do:
            EnviarDados(dataset XML_CONTRATANTES:handle).
            dataset XML_CONTRATANTES:empty-dataset ().
            in-conta = 0. 
        end.
