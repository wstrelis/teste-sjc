...
then do:
    ...
    /* Caso nenhum beneficiario esteja ativo no termo, nao simula os eventos de reembolso */
    FIND FIRST usuario WHERE usuario.cd-modalidade     = cd-modalidade-aux 
                         AND usuario.nr-ter-ade        = nr-ter-ade-aux   
                         AND usuario.dt-exclusao-plano = ?
                             NO-LOCK NO-ERROR.

    IF AVAIL usuario 
    THEN RUN logicaRN412.
