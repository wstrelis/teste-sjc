...
define temp-table CONTRATO no-undo
    ...
    field DT_INCLUSAO_CONTRATO as date 
    field DT_INICIO_CONTRATO as date 
    field DT_FIM_CONTRATO as date 
    field TIPO_COBRANCA as character .

define temp-table BENEFICIARIO no-undo
    field CODIGO as character 
        xml-node-type "ATTRIBUTE" 
    field CHAVE_SISTEMA as character 
    ...
    field GRAU_PARENTESCO as character 
    field DT_INICIO_COBERTURA as date 
    field DT_FIM_COBERTURA as date 
    field CHAVE_SISTEMA_PRODUTO_ANS as character 
    field CHAVE_SISTEMA_PRODUTO as character 
    field CONTRATO_id as recid 
        xml-node-type "HIDDEN" .

define dataset CONTRATOS  
    for CONTRATO, BENEFICIARIO
    parent-id-relation RELATION1 for CONTRATO, BENEFICIARIO
        parent-id-field CONTRATO_id.
...
function BuscaGrauParentesco returns integer 
    (in-grau-par                as   integer) forward.

function VerificaSituacaoContrato returns logical 
    (  ) forward.
...
procedure Executa: 
    ...
    run ExecutaQuery.
    EnviarDados(dataset CONTRATOS:handle).               
...
------------------------------------------------------------------------------*/
    ...
    define variable in-contagem         as   integer            no-undo.
    ...
    for each propost no-lock,
       ...
       where ti-pl-sa.cd-modalidade     = propost.cd-modalidade
        ...
        if available ter-ade then release ter-ade no-error.
        if available reg-plano-saude then release reg-plano-saude no-error.
        
        if propost.cd-sit-proposta < 5
        then do:
            if propost.dt-digitacao < 01/01/2015
            then next.
        end.        
        if propost.cd-sit-proposta >= 5
        then do:
            ...
            if not available ter-ade then next.
            
            if  ter-ade.dt-cancelamento    <> ?
            and ter-ade.dt-cancelamento     < 01/01/2010
            then next.                       
        end.        

        if propost.dt-atualizacao < date (temp-parametro-entrada.dt-qt-dias)
        then do:
            
            if not can-find (first usuario no-lock
                              where usuario.cd-modalidade   = propost.cd-modalidade
                                and usuario.nr-proposta     = propost.nr-proposta
                                and usuario.dt-atualizacao >= date (temp-parametro-entrada.dt-qt-dias))
            then next.                                
        end.

        find first reg-plano-saude no-lock
             where reg-plano-saude.idi-registro = propost.idi-plano-ans
                   no-error.
                   
        assign in-contagem = in-contagem + 1.         
        
        find first contrat where contrat.nr-insc-contratante = propost.nr-insc-contratante.
                
        create CONTRATO. 
        assign CONTRATO.CHAVE_SISTEMA                   = RegistroPrimary (buffer propost:handle)
               CONTRATO.CODIGO                          = substitute ('&1/&2', propost.cd-modalidade, propost.nr-proposta)
               CONTRATO.DT_INCLUSAO_CONTRATO            = propost.dt-digitacao
               CONTRATO.TIPO_COBRANCA                   = if propost.cd-forma-pagto = 1
                                                          then 'Prepagamento'
                                                          else if propost.cd-forma-pagto = 2
                                                               then 'CustoOperacional'
                                                               else 'Misto'
               CONTRATO.CHAVE_SISTEMA_CONTRATANTE       = RegistroPrimary (buffer contrat:handle).
               
        if  propost.nr-insc-contrat-origem <> ?
        and propost.nr-insc-contrat-origem <> 0
        then do:
            find first contrat where contrat.nr-insc-contratante    = propost.nr-insc-contrat-origem.
            assign CONTRATO.CHAVE_SISTEMA_RESPONSAVEL_FINANC        = RegistroPrimary (buffer contrat:handle).
        end. 
        else do:
            assign CONTRATO.CHAVE_SISTEMA_RESPONSAVEL_FINANC        = contrato.CHAVE_SISTEMA_CONTRATANTE.
        end.              
               
               
        if available ter-ade
        then do:
            assign CONTRATO.DT_INICIO_CONTRATO      = ter-ade.dt-inicio               
                   CONTRATO.DT_FIM_CONTRATO         = ter-ade.dt-cancelamento.
        ...
        for each usuario no-lock 
            ...
            if not available reg-plano-saude 
            then do:
                find first reg-plano-saude no-lock
                     where reg-plano-saude.idi-registro = usuario.idi-plano-ans
                           no-error.
                if not available reg-plano-saude then next.                           
            end.                  

            create BENEFICIARIO.
            assign BENEFICIARIO.CHAVE_SISTEMA               = RegistroPrimary (buffer usuario:handle)
                   BENEFICIARIO.CODIGO                      = substitute ('&1/&2/&3',
                                                                          usuario.cd-modalidade,
                                                                          usuario.nr-proposta,
                                                                          usuario.cd-usuario)
                   BENEFICIARIO.CHAVE_SISTEMA_PRODUTO       = RegistroPrimary (buffer ti-pl-sa:handle)
                   BENEFICIARIO.CHAVE_SISTEMA_PRODUTO_ANS   = RegistroPrimary (buffer reg-plano-saude:handle)
                   BENEFICIARIO.CONTRATO_id                 = recid (CONTRATO).
                   BENEFICIARIO.CPF                         = usuario.cd-cpf.
                   BENEFICIARIO.NOME                        = usuario.nm-usuario.
                   BENEFICIARIO.DT_NASCIMENTO               = usuario.dt-nascimento. 
                   BENEFICIARIO.GRAU_PARENTESCO             = string (BuscaGrauParentesco (usuario.cd-grau-parentesco )).
                   BENEFICIARIO.DT_INICIO_COBERTURA         = usuario.dt-inclusao-plano.
                   BENEFICIARIO.DT_FIM_COBERTURA            = usuario.dt-exclusao-plano.      
        end.
        ...
        do:                        
            ...
            then do: 
                EnviarDados(dataset CONTRATOS:handle).
                dataset CONTRATOS:empty-dataset ().
                in-contagem = 0. 
            end.
        end.
        catch cl-exception as Progress.Lang.Error :
            dataset CONTRATOS:empty-dataset ().
            in-contagem = 0. 
        end catch.
    end.   
...
function BuscaGrauParentesco returns integer 
    (in-grau-par                as   integer  ):
/*------------------------------------------------------------------------------
 Purpose:
 Notes:           
------------------------------------------------------------------------------*/    

    if in-grau-par  = 1 then return 1. /* TITULAR */
    if in-grau-par >= 2 and in-grau-par    <= 4 then return 2. /* CONJUJE */     
    if in-grau-par >= 5 and in-grau-par    <= 15 then return 3. /* FILHOS */
    if in-grau-par  = 17 then return 4. /* FILHO MAIOR 24 ANOS */
    if in-grau-par  = 50 or in-grau-par = 51 then return 5. /* PAIS */ 
...
------------------------------------------------------------------------------*/    
    ...
    if propost.nr-ter-adesao    = 0
    then do:
        assign CONTRATO.STATUS1 = "0".
        return yes.
    end.               
       
    if propost.nr-ter-adesao   <> 0 
    then do:
        assign CONTRATO.STATUS1 = "1".
        
        if CONTRATO.DT_FIM_CONTRATO = ?
        or CONTRATO.DT_FIM_CONTRATO > today
        then do:
            assign CONTRATO.STATUS1 = "1".
            return yes.
        end.
        if propost.cd-sit-proposta  = 3
        then do:
            assign CONTRATO.STATUS1 = "2".
            return yes.
        end.
        if propost.cd-sit-proposta  = 90
        or ter-ade.dt-cancelamento <= today
        then do:
            assign CONTRATO.STATUS1 = "4".
            return yes.
        end.
