...
function PossuiBenefAtivoPeriodo returns logical 
    (in-modalidade              as   integer,
     in-termo-adesao            as   integer,
     dt-inicio                  as   date,
     dt-fim                     as   date) forward.

function VerificaEventoProporcionalProximoPeriodo returns logical 
    (in-modalidade as integer,
     in-termo-adesao as integer,
     in-ano as integer,
     in-mes as integer) forward.
...
------------------------------------------------------------------------------*/
    ...
    for each propost no-lock
       where propost.cd-modalidade      > 0
         and propost.cd-plano           > 0
         and propost.cd-tipo-plano      > 0
         and propost.cd-forma-pagto     = 1,
       first ter-ade no-lock
       where ter-ade.cd-modalidade      = propost.cd-modalidade
         and ter-ade.nr-ter-adesao      = propost.nr-ter-adesao: 
        
        if ter-ade.dt-inicio    = ter-ade.dt-cancelamento
        then next. 
        ...
        for each notaserv no-lock
           where notaserv.cd-modalidade       = propost.cd-modalidade
            ...
            then do:
                ...
                create XML_ALERTA.              
                assign XML_ALERTA.CHAVE_SISTEMA             = "PP" + RegistroPrimary (buffer notaserv:handle)
            ...
            then do:
                ...
                if notaserv.cd-contratante-origem = 0
                then do:
                    ...
                    find first buf-contrat-orig no-lock
                         where buf-contrat-orig.nr-insc-contratante = notaserv.cd-contratante-origem.
                         
                    find first fatura no-lock
                         where fatura.cd-contratante    = buf-contrat-orig.cd-contratante
                           and fatura.nr-fatura         = notaserv.nr-fatura
                               no-error.
...
/*------------------------------------------------------------------------------
 ...
 Notes:
------------------------------------------------------------------------------*/    
    define variable lg-possui-ativo             as   logical        no-undo.

    for each usuario no-lock
       where usuario.cd-modalidade        = in-modalidade
         and usuario.nr-ter-adesao        = in-termo-adesao:
        
        if usuario.cd-sit-usuario   >= 90
        then next.
        
        if usuario.dt-inclusao-plano    > dt-fim
        then next.
        
        if usuario.dt-exclusao-plano   <> ?
        and usuario.dt-exclusao-plano   < dt-inicio
        then next.
        
        find first suspusua no-lock
             where suspusua.cd-modalidade           = usuario.cd-modalidade
               and suspusua.nr-ter-adesao           = usuario.nr-ter-adesao
               and suspusua.cd-usuario              = usuario.cd-usuario
               and suspusua.dt-inicio-suspensao    <= dt-inicio
               and suspusua.dt-fim-suspensao       >= dt-fim
                   no-error.

        if available suspusua
        then next.                   
        
        assign lg-possui-ativo = yes.
        leave.
             
    end.             

    return lg-possui-ativo.
...
------------------------------------------------------------------------------*/
    ...
    if in-mes = 13 then assign in-mes = 1 in-ano = in-ano + 1. 

    for each notaserv no-lock
       where notaserv.cd-modalidade       = in-modalidade
         and notaserv.nr-ter-adesao       = in-termo-adesao
         and notaserv.aa-referencia       = in-ano
         and notaserv.mm-referencia       = in-mes
         and (   notaserv.in-tipo-nota    = 0
              or notaserv.in-tipo-nota    = 5):
        ...
        for each vlbenef no-lock
           ...
           where evenfatu.in-entidade   = 'FT'
             ...
             and (   evenfatu.in-classe-evento  = 'L'
