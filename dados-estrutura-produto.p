...
/*------------------------------------------------------------------------
    File        : dados-estrutura-produto.i
    Purpose     : 
    ...
    Author(s)   : 
    Created     : Thu Jun 14 13:53:31 BRT 2018
    Notes       :
  ----------------------------------------------------------------------*/
...
/* ***************************  Main Block  *************************** */
define temp-table XML_ESTRUTURA_PRODUTO no-undo
    ...
    field TIPO_CONTRATACAO as character .

define temp-table XML_ESTRUTURA_PRODUTO_ANS no-undo
    namespace-uri "http://wwww.thealth.com.br" 
    field CODIGO as character 
        xml-node-type "ATTRIBUTE" 
    field CHAVE_SISTEMA as character 
...
define dataset XML_ESTRUTURA_PRODUTOS namespace-uri "http://wwww.thealth.com.br" 
    for XML_ESTRUTURA_PRODUTO, XML_ESTRUTURA_PRODUTO_ANS.


define variable SEGMENTO_REGULAMENTADO      as   character  init "Regulamentado"        no-undo.
define variable SEGMENTO_NAO_REGULAMENTADO  as   character  init "NaoRegulamentado"     no-undo.
define variable SEGMENTO_ADAPTADO           as   character  init "Adaptado"             no-undo.
define variable SEGMENTO_NAO_INFORMADO      as   character  init "NaoInformado"         no-undo.

define variable TIPO_CONTRAT_INDIVIDUAL         as   character  init "Individual"       no-undo.
define variable TIPO_CONTRAT_COLETIVO_ADESAO    as   character  init "ColetivoAdesao"   no-undo.
define variable TIPO_CONTRAT_EMPRESARIAL        as   character  init "Empresarial"      no-undo.
define variable TIPO_CONTRAT_NAO_INFORMADO      as   character  init "NaoInformado"     no-undo.
...
------------------------------------------------------------------------------*/
    for each ti-pl-sa no-lock:
        ...
        create XML_ESTRUTURA_PRODUTO.
        assign XML_ESTRUTURA_PRODUTO.CHAVE_SISTEMA      = RegistroPrimary (buffer ti-pl-sa:handle)
               XML_ESTRUTURA_PRODUTO.CODIGO             = substitute ("&1/&2/&3",
                                                                      string (ti-pl-sa.cd-modalidade, '99'),
                                                                      string (ti-pl-sa.cd-plano, '99'),
                                                                      string (ti-pl-sa.cd-tipo-plano, '99'))
               XML_ESTRUTURA_PRODUTO.DESCRICAO          = ti-pl-sa.nm-tipo-plano
               XML_ESTRUTURA_PRODUTO.SEGMENTO           = SEGMENTO_NAO_INFORMADO
               XML_ESTRUTURA_PRODUTO.TIPO_CONTRATACAO   = TIPO_CONTRAT_NAO_INFORMADO.
                       
        if ti-pl-sa.int-2   = 1
        then XML_ESTRUTURA_PRODUTO.TIPO_CONTRATACAO = TIPO_CONTRAT_INDIVIDUAL.

        if ti-pl-sa.int-2   = 3
        or ti-pl-sa.int-2   = 4         
        then XML_ESTRUTURA_PRODUTO.TIPO_CONTRATACAO = TIPO_CONTRAT_COLETIVO_ADESAO.

        if ti-pl-sa.int-2   = 5
        or ti-pl-sa.int-2   = 6         
        then XML_ESTRUTURA_PRODUTO.TIPO_CONTRATACAO = TIPO_CONTRAT_EMPRESARIAL.

               
        find first reg-plano-saude no-lock
             where reg-plano-saude.idi-registro = ti-pl-sa.idi-plano-ans
                   no-error.
                   
        if not available reg-plano-saude then next.
        
        if reg-plano-saude.in-tipo-regulamentacao   = 1
        then do:
            XML_ESTRUTURA_PRODUTO.SEGMENTO  = SEGMENTO_REGULAMENTADO.
        end.
        
        if reg-plano-saude.in-tipo-regulamentacao   = 2
        then do:
            XML_ESTRUTURA_PRODUTO.SEGMENTO  = SEGMENTO_NAO_REGULAMENTADO.
        end.
        
        if reg-plano-saude.in-tipo-regulamentacao   = 3
        then do:
            XML_ESTRUTURA_PRODUTO.SEGMENTO  = SEGMENTO_ADAPTADO.
        end.
    end.
end procedure.

procedure BuscaProdutosANS:
/*------------------------------------------------------------------------------
 Purpose:
 Notes:
------------------------------------------------------------------------------*/
    for each reg-plano-saude no-lock:
        
        create XML_ESTRUTURA_PRODUTO_ANS.
        
        assign XML_ESTRUTURA_PRODUTO_ANS.CHAVE_SISTEMA      = RegistroPrimary(buffer reg-plano-saude:handle)
               XML_ESTRUTURA_PRODUTO_ANS.DESCRICAO          = reg-plano-saude.nom-plano-ans.
               
        if reg-plano-saude.in-tipo-regulamentacao   = 1
        then do:
            assign XML_ESTRUTURA_PRODUTO_ANS.CODIGO     = string (reg-plano-saude.cdn-plano-ans).
        end.     
        
        if reg-plano-saude.in-tipo-regulamentacao   = 2        
        then do:
            assign XML_ESTRUTURA_PRODUTO_ANS.CODIGO     = string (reg-plano-saude.cod-plano-operadora).
        end.     
...
procedure Executa:
    ...
    run BuscaProdutos.
    run BuscaProdutosANS.
    
    EnviarDados(dataset XML_ESTRUTURA_PRODUTOS:handle ).
end procedure.
