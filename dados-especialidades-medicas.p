...
define temp-table XML_ESPECIALIDADE no-undo
    namespace-uri "http://www.thealth.com.br/MedicalSpeciality.xsd" 
    field CODIGO as character 
        xml-node-type "ATTRIBUTE" 
    field CHAVE_SISTEMA as character 
    field DESCRICAO as character .

define dataset XML_ESPECIALIDADES namespace-uri "http://www.thealth.com.br/MedicalSpeciality.xsd" 
    for XML_ESPECIALIDADE.
...
procedure Executa:
    ...
    log-manager:write-message (substitute ('modo de execuá∆o: &1', CH-MODO), 'DEBUG').
    ...
    EnviarDados (dataset XML_ESPECIALIDADES:handle).
...
------------------------------------------------------------------------------*/
    ...
    log-manager:write-message (substitute ('iniciando processamento, data de atualizaá∆o limite: &1', 'DEBUG')).
    
    for each esp-med no-lock:
        
        if esp-med.dt-atualizacao  < dt-alterados-desde
        then next.
        
        create XML_ESPECIALIDADE.
        assign XML_ESPECIALIDADE.CHAVE_SISTEMA  = RegistroPrimary (buffer esp-med:handle)
               XML_ESPECIALIDADE.CODIGO         = string (esp-med.cd-especialid, '999')
               XML_ESPECIALIDADE.DESCRICAO      = esp-med.ds-especialid.
    end.
