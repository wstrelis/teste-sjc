...
define temp-table XML_INSUMO no-undo
    namespace-uri "http://www.thealth.com.br/MedicalSupply.xsd" 
    field CODIGO as character 
        xml-node-type "ATTRIBUTE" 
    field DESCRICAO as character 
    field CHAVE_SISTEMA as character 
    field CHAVE_SISTEMA_TIPO_INSUMO as character .

define dataset XML_INSUMOS namespace-uri "http://www.thealth.com.br/MedicalSupply.xsd" 
    for XML_INSUMO.
...
procedure Executa:
    ...
    define variable dt-ativos-em                as   date           no-undo.
    ...
    EnviarDados (dataset XML_INSUMOS:handle).
...
------------------------------------------------------------------------------*/
    ...
    for each insumos no-lock:
        
        if insumos.dt-atualizacao  < dt-alterados-desde
        then next.
        
        if insumos.dat-fim-vigenc < 01/01/2016 then next.
        
        find first tipoinsu no-lock
             where tipoinsu.cd-tipo-insumo  = insumos.cd-tipo-insumo
                   no-error.
                   
        if not available tipoinsu then next.
        
        create xml_insumo.
        assign in-conta                             = in-conta + 1
               xml_insumo.CHAVE_SISTEMA             = RegistroPrimary (buffer insumos:handle)
               xml_insumo.CHAVE_SISTEMA_TIPO_INSUMO = RegistroPrimary (buffer tipoinsu:handle)
               xml_insumo.CODIGO                    = substitute ('&1&2',
                                                                  string (insumos.cd-tipo-insumo, '99'),
                                                                  string (insumos.cd-insumo, '99999999'))
               xml_insumo.DESCRICAO                 = insumos.ds-insumo.
        ...
        then do:
            EnviarDados(dataset XML_INSUMOS:handle).
            in-conta = 0.
            dataset XML_INSUMOS:empty-dataset ().
        end.               
    end.
